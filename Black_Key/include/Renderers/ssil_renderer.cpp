...
void SSGIRenderer::Init(VulkanEngine* engine)
{
...
void SSGIRenderer::ConfigureRenderWindow()
{
...
void SSGIRenderer::InitSwapchain()
{
	CreateSwapchain(_windowExtent.width, _windowExtent.height);
}

void SSGIRenderer::InitRenderTargets()
{
	...
	VkImageUsageFlags gBufferImageUsages{};
	gBufferImageUsages |= VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
	gBufferImageUsages |= VK_IMAGE_USAGE_STORAGE_BIT;
	gBufferImageUsages |= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
	gBufferImageUsages |= VK_IMAGE_USAGE_SAMPLED_BIT;


	VkImageCreateInfo gbuffer_img_info = vkinit::image_create_info(_drawImage.imageFormat, drawImageUsages, drawImageExtent, 1, msaa_samples);
	...
	vmaCreateImage(engine->_allocator, &gbuffer_img_info, &rimg_allocinfo, &_drawImage.image, &_drawImage.allocation, nullptr);
...
void ClusteredForwardRenderer::InitComputePipelines()
{
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/cluster_cull_light_shader.spv").c_str(), engine->_device, &cullLightShader)) {
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/indirect_cull.comp.spv").c_str(), engine->_device, &cullObjectsShader)) {
		std::print("Error when building the compute shader \n");
	}
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/depth_reduce.comp.spv").c_str(), engine->_device, &depthReduceShader)) {
		std::print("Error when building the compute shader \n");
	}
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/downsample.spv").c_str(), engine->_device, &downsample_shader)) {
		std::print("Error when building the compute shader \n");
	}
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/upsample.spv").c_str(), engine->_device, &upsample_shader)) {
		std::print("Error when building the compute shader \n");
	}
...
void ClusteredForwardRenderer::LoadAssets()
{
	...
	std::string cubePath{ assets_path + "/models/cube.gltf" };
	auto cubeFile = resource_manager->loadGltf(engine, cubePath);
	assert(cubeFile.has_value());

	std::string planePath{ assets_path + "/models/plane.glb" };
...
void ClusteredForwardRenderer::BuildClusters()
{
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/cluster_shader.spv").c_str(), engine->_device, &buildClusterShader)) {
		std::print("Error when building the compute shader \n");
	}
...
void ClusteredForwardRenderer::GeneratePrefilteredCubemap()
{
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/filter_cube.vert.spv").c_str(), engine->_device, &preFilterVertexShader)) {
		std::print("Error when building the shadow vertex shader module\n");
	}

	VkShaderModule preFilterFragmentShader;
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/pre_filter_envmap.frag.spv").c_str(), engine->_device, &preFilterFragmentShader)) {
...
void ClusteredForwardRenderer::GenerateIrradianceCube()
{
	...
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/filter_cube.vert.spv").c_str(), engine->_device, &irradianceVertexShader)) {
		std::print("Error when building the shadow vertex shader module\n");
	}

	VkShaderModule irradianceFragmentShader;
	if (!vkutil::load_shader_module(std::string(assets_path + "/shaders/irradiance_cube.frag.spv").c_str(), engine->_device, &irradianceFragmentShader)) {
		std::print("Error when building the shadow fragment shader module\n");
	}
